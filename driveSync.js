...
function semanaGrupo(nombre) {
  ...
  let diasAlMier  = (3 - primerDia.getDay() + 7) % 7;
  // Si el día 1 ya es miércoles, diasAlMier=0 → correcto (sem1 empieza el día 1)
  const primerMier = new Date(año, mes-1, 1 + diasAlMier);
  const ini = new Date(primerMier); ini.setDate(primerMier.getDate() + (numSem-1)*7);
