...
function semanaAsistencias(p) {
  ...
  const diasAlSab=dia===6?0:(dia===0?6:6-dia);
  const sab=new Date(fin);sab.setDate(fin.getDate()+diasAlSab);
...
function semanaGrupo(nombre) {
  ...
  if(!mes)return null;
  const p=new Date(año,mes-1,1),dAlM=(3-p.getDay()+7)%7;
  const pm=new Date(año,mes-1,1+dAlM);
...
async function extraerDatos(drive, fileId, tipo) {
  ...
  const prompts={
    ventas_mesero:`Extrae datos de ventas por mesero de este PDF SoftRestaurant. SOLO JSON array:\n[{"nombre":"...","venta":número,"prop_tarjeta":número,"efectivo":número,"comensales":número}]`,
    ventas_grupo:`Extrae ventas por grupo/categoría. SOLO JSON array:\n[{"grupo":"...","venta":número,"porcentaje":número}]`,
    asistencias:`Extrae asistencia de empleados. SOLO JSON array:\n[{"nombre":"...","horas_reales":número,"dias_asistidos":número}]`
  };
  const msg=await ai.messages.create({model:"claude-opus-4-5-20251101",max_tokens:2000,messages:[{role:"user",content:[
...
async function syncSemanal() {
  ...
  // 1. Ventas mesero (MIÉ-DOM desde nombre del PDF)
  console.log("[SYNC] Ventas/mesero...");
  ...
  // 2. Ventas grupo (sem N MMMM → MIÉ-DOM)
  console.log("[SYNC] Ventas/grupo...");
  ...
  // 3. Asistencias (DOM-SAB desde nombre del PDF)
  console.log("[SYNC] Asistencias...");
  ...
  // Limpiar claves incorrectas anteriores
